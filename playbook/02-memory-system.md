...
3. **Project files** — structured tracking for multi-day work
...
2. Hourly, extracts new observations from daily note files
...
SQLite observation database
         ↓ (on session start)
MEMORY.md (top ~50 observations, auto-generated)
```

The extraction process parses daily notes, identifies distinct observations (decisions, discoveries, changes, errors), and stores them with metadata (timestamp, category, importance score).
...
Build your own `mem-search` script that queries the SQLite database. The schema is straightforward:

```sql
-- Observations table (approximate — adapt to your needs)
CREATE TABLE observations (
    id INTEGER PRIMARY KEY,
    content TEXT NOT NULL,
    category TEXT,           -- discovery, decision, change, error, context
    importance REAL,         -- 0.0 to 1.0
    source_file TEXT,        -- which daily note it came from
