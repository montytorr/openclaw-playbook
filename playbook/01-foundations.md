...
│   └── .dreams/            # Dreaming/recall artifacts, if enabled
...
### DREAMS.md — Dream Diary / Synthesis Surface

If you enable OpenClaw's dreaming or higher-level memory synthesis, keep `DREAMS.md` as the readable diary surface and `memory/.dreams/` as the artifact store. Treat it as reflection over memory, not the authoritative memory store. Daily notes and project files remain the factual source of truth.

In multi-root workspaces, document which copy is runtime-canonical and keep mirrors aligned during audits.
...
| DREAMS.md | ✅ | ✅ / generated | Optional dream diary; align mirrors if your runtime uses them |
